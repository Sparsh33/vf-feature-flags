"""Anthropic Claude integration for the NL flag builder.

Uses Claude Sonnet 4.6 with tool use for structured extraction and prompt
caching on system prompt + tool definitions.
"""
...
async def chat_turn(
    ...
    for block in response.content:
        ...
        if block_type == "text":
            ...
            if tool_name == "update_flag_draft":
                _merge_draft(merged_draft, tool_input)
            elif tool_name == "confirm_flag":
                confirm_called = True
...
def _merge_draft(target: Dict[str, Any], source: Dict[str, Any]) -> None:
